...
public class DefaultMemberService implements MemberService {
    ...
    public MemberCreationResponseDto createMember(MemberCreationRequestDto request, String oidcId) {
        ...
        // 제로투원 운영 페이지를 방문했을 때 최초 회원 가입 시 members/{memberId}/profile 경로의 memberId가 NaN으로 뜨는 치명적인 버그가 있었는데,
        ...
        if (!FormatValidator.hasValue(memberId)) {
            throw new MemberIdNoValueException(memberId);
        }
        ...
        String profileImageUploadUrl = fileUrlResolver.generateFileUploadUrl(
                // 변경 사유: 프로필 생성 엔드포인트가 MemberProfileService와 중복됩니다. 분리하여 재사용하는 편이 낫다고 생각됩니다.
